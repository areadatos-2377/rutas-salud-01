# Rutas_01 — instrucciones para asistentes de IA
...
Repo de trabajo de dos personas: **jesus_sam** y **jorge**, cada quien puede usar un asistente de IA distinto (Claude Code, Codex u otro). Este archivo aplica **sin importar qué asistente lo esté leyendo ni quién esté trabajando** — síguelo siempre, no solo cuando alguien lo mencione explícitamente.
...
2. Lee la entrada más reciente de **ambos** archivos en `docs/bitacora/` (`jesus_sam.md` y `jorge.md`) — basta la más reciente de cada uno, no hace falta el historial completo. Esto da contexto de qué hizo la otra persona recientemente, bloqueadores activos y si algo que vas a tocar depende de o afecta su trabajo.
...
## Uso de subagentes (si tu herramienta los soporta)

Si tu asistente permite lanzar subagentes/agentes auxiliares con contexto propio, sigue `legacy/protocolo_multiagentes.md` para cuándo y cómo — tipos de agente, prompts autocontenidos, y cuándo NO usarlos (tareas triviales, cambios pequeños, resultados secuenciales dependientes). Ese documento fue escrito pensando en Claude Code; si tu herramienta tiene su propio mecanismo de sub-tareas, adapta la lógica (cuándo delegar, cómo estructurar el prompt) aunque los nombres de agente no coincidan.
...
## Si tu herramienta no lee este archivo automáticamente

Este archivo (`AGENTS.md`) es una convención abierta que ya siguen varias herramientas de código con IA — pero no es universal. Si la persona que se suma usa un asistente que no lo carga solo, basta con que en su primera sesión le pida algo como *"lee AGENTS.md y síguelo durante todo el trabajo en este repo"* una vez — después de eso, si su herramienta soporta memoria/instrucciones persistentes propias, puede guardarlo ahí.

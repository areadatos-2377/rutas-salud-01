...
Antes de esto tuve que reconocer un descuido: la entrada (15) de esta bitácora ya daba por hecho un selector de entidad y las 11 columnas en `JornadaDetallePage`, pero ese commit (`881ce29`, selector de entidad) nunca actualizó bitácora ni `.context/progress.md` — se quedó sin registrar hasta ahora. El usuario notó la desconexión ("no veo esos cambios en local, seguro que los subiste?") cuando en realidad yo había creado un archivo de utilidades sin comittear y me había desviado a otra tarea (probar `tools/captura-programacion/` con Playwright, que sí quedó bien documentado en su propio momento) sin volver a esto. Lección operativa: si me desvío de una tarea a mitad de camino, dejar explícito en la próxima respuesta qué quedó a medias, no asumir que "ya lo mencioné antes" es suficiente.

Probado con Playwright real, dos roles: como `usuario_entidad` (colima1) — agregar unidad a una ruta existente, verificar autocompletado de CLUES y conteo, eliminar esa unidad, crear una ruta nueva (sin selector de entidad, como corresponde), agregar una unidad a esa ruta nueva, eliminar la ruta completa (cascada correcta); como `super_admin` (super) — confirmar que el formulario de "Agregar ruta" sí muestra el selector de entidad, crear una ruta para una entidad que no tenía ninguna en esa jornada (Baja California), confirmar que aparece como grupo nuevo, eliminarla. **Nota de transparencia:** durante estas pruebas desapareció una ruta de prueba preexistente ("Rita 01", Campeche, 0 unidades) que mi script no tocó — un re-test aislado confirmó que el código de eliminar-ruta funciona correctamente (1 grupo matcheado, 1 botón, 1 DELETE, sin efectos colaterales), así que lo más probable es que el usuario la haya eliminado él mismo explorando la función en su propio navegador (Vite recarga en caliente en cuanto guardo el archivo). Se lo señalé directamente en vez de asumir.

**Bloqueadores activos:** Ninguno.
**Depende de / afecta a:** Si Jorge toca `NIVELES_POR_CATEGORIA` o `CATEGORIA_LABEL`, ahora viven en `frontend/src/utils/categoriaNiveles.js` — no los vuelva a copiar en una página nueva.
**Próximo:** A decidir con el usuario.
